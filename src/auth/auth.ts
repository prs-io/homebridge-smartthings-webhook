...
export class SmartThingsAuth {
  ...
  public async handleOAuthCallback(query: any, res: http.ServerResponse): Promise<void> {
    try {
      ...
      res.end('<h1>Authentication successful!</h1><p>You can close this window and click Save in Homebridge UI.</p>');
  ...
  public startAuthFlow(): void {
    ...
    if (this.platform.config.server_url && this.platform.config.server_url.trim() !== '') {
      ...
      // Save auth URL to file for UI to read
      const authUrlFile = path.join(this.platform.api.user.storagePath(), 'smartthings_auth_url.txt');
